...
```{r Save RDS, eval=T}
# save SCE
saveRDS(sce, file = paste0("../data/fresh_data/merged_SCE_data_sources.rds"))
...
saveRDS(reads_per_analysis_stage, file = "../data/fresh_data/EDI_reads_per_stage.rds")

# Save reads per stage information
cells_per_analysis_stage <- rbind(cells_per_analysis_stage1, cells_per_analysis_stage2, cells_per_analysis_stage3)
saveRDS(cells_per_analysis_stage, file = "../data/fresh_data/EDI_cells_per_stage.rds")
```


```{r Read in rds}
# load SCE
sce <- readRDS(file = paste0("../data/fresh_data/merged_SCE_data_sources.rds"))

# load reads per stage information
reads_per_analysis_stage <- readRDS(file = paste0("../data/fresh_data/EDI_reads_per_stage.rds"))
cells_per_analysis_stage <- readRDS(file = paste0("../data/fresh_data/EDI_cells_per_stage.rds"))
```
...
```{r Add QC metrics}
# change the rownames to the gene symbol (instead of the enembl gene id). This makes the data easier to read.
...
suppressWarnings(figure10)

ggsave(paste0(folder_path, "figure10.png"),
       plot=figure10,
       width=20*0.9,
       height=26*0.9,
       units="in",
