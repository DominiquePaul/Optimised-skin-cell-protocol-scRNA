...
```{r load data}
sce <- readRDS(file="../data/fresh_data/protocol_integration_analysis_sce_EOS2.rds")
```
...
<!-- ```{r batch correction fastMNN, fig.width=18, fig.height=12, eval=FALSE} -->
<!-- cat("\n\n##  Run batch correction with fastMNN \n\n") -->


<!-- # we  -->
<!-- sce <- batchelor::multiBatchNorm(sce,  -->
<!--                                       batch=sce$Sample, -->
<!--                                       subset.row = rowData(sce)$is_hvg, -->
<!--                                       normalize.all=TRUE, -->
<!--                                       BPPARAM = bpparam) -->

<!-- merge_order <- list(list("Lafa_S1","Lafa_S124","Lafa_S125","Lafa_S18","Lafa_S32","Lafa_S33","Lafa_S34","Lafa_S4","Lafa_S50","Lafa_S68"), # Lafayatis -->
<!--                     list("Sole_Boldo_S396", "Sole_Boldo_S397", "Sole_Boldo_S398", "Sole_Boldo_S399", "Sole_Boldo_S400"), -->
<!--                     list("He_S62", "He_S64", "He_S67", "He_S68", "He_S70", "He_S71", "He_S75"), -->
<!--                     # list("SSc_S1","SSc_S2","SSc_S4","SSc_S5"), # SSc samples -->
<!--                     list("Fresh_S1","Fresh_S2") # Fresh samples -->

<!--                     ) -->

<!-- # Correct batch effects with fastMNN -->
<!-- ## we should only use the corrected values for visualisation -->
<!-- set.seed(100) -->
<!-- fast.mnn <- fastMNN(sce, d=50, batch=factor(sce$Sample), correct.all=TRUE, subset.row=rowData(sce)$is_hvg, -->
<!--                     merge.order=merge_order) -->

<!-- reducedDim(sce, "corrected") <- reducedDim(fast.mnn, "corrected")    -->
<!-- assay(sce, "reconstructed") <- assay(fast.mnn, "reconstructed")         -->

<!-- set.seed(100) -->
<!-- sce <- runUMAP(sce, dimred="corrected", subset_row=rowData(sce)$is_hvg, n_neighbors=50, min_dist = 0.1, name="UMAP_fastMNN") -->

<!-- cat("\n\n### UMAP corrected by sample\n\n") -->
<!-- print(plotReducedDim(sce, "UMAP_fastMNN", colour_by="Sample", point_size=0.25) + -->
<!--   ggtitle("UMAP post batch correction - fastMNN") + -->
<!--   guides(colour=guide_legend(override.aes=list(size=4))) + -->
<!--     theme_ipsum_rc()) -->

<!-- cat("\n\n### UMAP corrected by protocol \n\n") -->
<!-- print(plotReducedDim(sce, "UMAP_fastMNN", colour_by="Protocol", point_size=0.25) + -->
<!--   ggtitle("UMAP post batch correction - fastMNN") + -->
<!--   guides(colour=guide_legend(override.aes=list(size=4))) + -->
<!--     theme_ipsum_rc()) -->
...
```{r cellmixs batchelor, fig.width=14, fig.height=6}
# bpstart(bpparam)
...
kgraph_clusters_walktrap <- clusterRows(reducedDim(sce, "harmony"),
    TwoStepParam(
        first=KmeansParam(centers=10000, nstart = 5, iter.max = 100),
        second=NNGraphParam(k=30, cluster.fun="walktrap", BPPARAM=bpparam)
    )
)
# bpstop(bpparam)
toc(log=TRUE)
...
<!-- ```{r clustering fastmnn, results="asis", eval=FALSE} -->
<!-- cat("\n\n## Two step clustering based on fastMNN integration\n\n") -->

<!-- tic("kgraph_clusters fastmnn") -->
<!-- set.seed(100) -->
<!-- # bpstart(bpparam) -->
<!-- kgraph_clusters <- clusterRows(reducedDim(sce, "corrected"), -->
<!--     TwoStepParam( -->
<!--         first=KmeansParam(centers=10000, nstart = 5, iter.max = 100), -->
<!--         second=NNGraphParam(k=10, cluster.fun="louvain", BPPARAM=bpparam) -->
<!--     ) -->
<!-- ) -->
<!-- # bpstop(bpparam) -->
<!-- toc(log=TRUE) -->

<!-- tic("kgraph_clusters_walktrap fastmnn") -->
<!-- set.seed(100) -->
<!-- # bpstart(bpparam) -->
<!-- kgraph_clusters_walktrap <- clusterRows(reducedDim(sce, "corrected"), -->
<!--     TwoStepParam( -->
<!--         first=KmeansParam(centers=10000, nstart = 5, iter.max = 100), -->
...
<!-- colData(sce)$kgraph_clusters_fastMNN <- factor(kgraph_clusters) -->
<!-- colData(sce)$kgraph_clusters_walktrap_fastMNN <- factor(kgraph_clusters_walktrap) -->
<!-- ``` -->
...
<!-- ```{r, fig.width=18, fig.height=12, results="asis", eval=FALSE} -->
<!-- cat("\n\n## UMAP (fastMNN) {.tabset}\n\n") -->

<!-- cat("\n### K-Graph \n\n") -->
<!-- plotReducedDim(sce, "UMAP_fastMNN", colour_by="kgraph_clusters_fastMNN", text_by="kgraph_clusters_fastMNN", point_size=0.5) + -->
<!--   ggtitle("Two step k-means clusters") + -->
<!--   theme_ipsum_rc() -->

<!-- cat("\n### K-Graph by Protocol\n\n") -->
<!-- plotReducedDim(sce, "UMAP_fastMNN", colour_by="kgraph_clusters_fastMNN", other_fields="Protocol", point_size=0.25) + -->
<!--   ggtitle("Two step k-means clusters") + -->
<!--   facet_wrap(~Protocol) + -->
<!--   theme_ipsum_rc() -->

<!-- cat("\n### K-Graph Walktrap \n\n") -->
<!-- plotReducedDim(sce, "UMAP_fastMNN", colour_by="kgraph_clusters_walktrap_fastMNN", text_by="kgraph_clusters_walktrap_fastMNN", point_size=0.5) + -->
<!--   ggtitle("Two step walktrap clusters") + -->
<!--   theme_ipsum_rc() -->

<!-- cat("\n### K-Graph Walktrap by protocol\n\n") -->
<!-- plotReducedDim(sce, "UMAP_fastMNN", colour_by="kgraph_clusters_walktrap_fastMNN", other_fields="Protocol", point_size=0.25) + -->
<!--   ggtitle("Two step walktrap clusters") + -->
<!--   facet_wrap(~Protocol) + -->
<!--   theme_ipsum_rc() -->
<!-- ``` -->
...
<!-- ```{r fastMNN barplots, fig.width=14, fig.height=10, results="asis", eval=FALSE} -->
<!-- cat("\n\n## fastMNN {.tabset}\n\n") -->

<!-- plot_label_barplots(sce, "kgraph_clusters_fastMNN", tabset="###") -->

<!-- plot_label_barplots(sce, "kgraph_clusters_walktrap_fastMNN", tabset="###") -->
<!-- ``` -->
...
```{r}
saveRDS(sce, file = "../data/fresh_data/protocol_integration_analysis_sce_EOS3.rds")
```
