...
```{r load-data}
sce <- readRDS(file="../data/fresh_data/Protocol2_All_sce_EOS4.rds")
...
Expr_avg_to_overall_avg = Average expression value of gene in e.g. keratinocytes of Sample X  <!-- / Average expression value of gene in all other cells of sample X -->
...
genes_ball_plot_with_values <- lapply(unique(sce$Sample), function(sample){
  ...
  # # same calculations for all cells  
  # df_intermediate_all_cells <- genes_ball_plot
  # df_intermediate_all_cells$sample <- sample
  # df_intermediate_all_cells$mean_of_logs <- rowMeans(logcounts(sce[,sce$Sample == sample])[genes_ball_plot$gene,])
  # # df_intermediate_all_cells$frac_cells_expressed_in <- rowMeans(assay(sce[,sce$Sample == sample])[genes_ball_plot$gene,] > 0)
  # df_intermediate_all_cells
  # 
  # df_intermediate_level1 %>%
  #   left_join(df_intermediate_all_cells[,c("gene","mean_of_logs")], by="gene")
