...
```{r load-data}
sce <- readRDS(file="../data/fresh_data/protocol_integration_analysis_sce_EOS5.rds")
sce$Protocol <- sub("Tabib et al.", "Tabib_et_al", sce$Protocol)
sce$Protocol <- sub("Solé-Boldo et al", "Sole_Boldo_et_al", sce$Protocol)
sce$Protocol <- sub("He et al.", "He_et_al", sce$Protocol)
sce$Protocol <- sub("SSc", "Cultured", sce$Protocol)
sce$Protocol <- factor(sce$Protocol)
#saveRDS(sce, file=file_name)
```
...
combs_contr <- combn(unique(y.ab$samples[[test_col]]) ,2)
...
```{r Fresh vs. Lafa}
res_fresh_lafa <- glmQLFTest(fit.ab, contrast = contrast[,"Fresh-Tabib_et_al", drop=FALSE])
...
```{r Fresh vs. He}
res_fresh_he_et_al <- glmQLFTest(fit.ab, contrast = contrast[,"Fresh-He_et_al", drop=FALSE])
...
res_fresh_sole_boldo <- glmQLFTest(fit.ab, contrast = contrast[,"Fresh-Sole_Boldo_et_al", drop=FALSE])
