...
```{r load data}
sce_fresh <- readRDS("../data/fresh_data/Protocol2_Fresh_all_sce_EOS1.rds")
# sce_fresh <- readRDS(file="../data/fresh_data/Protocol2_Fresh_all_sce_EOS3.rds")
```
...
plotReducedDim(sce_fresh[,mask], "UMAP", colour_by="DCT") +
  labs(title="UMAP zoomed in on Melanocytes + Schwann cells",
       subtitle="Coloured by log values of DCT gene") +
  theme_ipsum_rc()

plotReducedDim(sce_fresh[,mask], "UMAP", colour_by="CDH19") +
  labs(title="UMAP zoomed in on Melanocytes + Schwann cells",
...
<!-- ```{r subcluster heatmaps, fig.width=10, fig.height=7, results="asis"} -->
...
<!--   column_annotations <- data.frame("cell_name"=rownames(colData(sce_fresh)), "Cluster"=factor(colData(sce_fresh)[,cell_type])) %>% -->
<!--     filter(Cluster!="NA") %>% -->
<!--     arrange(Cluster) %>% -->
<!--     tibble::column_to_rownames(var="cell_name") -->

<!--   # the logcounts which we need for the pheatmap function -->
<!--   sce_plot <- assay(sce_fresh[genes, rownames(column_annotations)], "logcounts") -->
...
<!--     labs(title=paste0("Marker genes for ", cell_type))) -->
<!-- } -->

<!-- cell_types_subclustering <- unique(heatmap_marker_genes$Group) -->
<!-- # remove "coarse" from the list. We only want the cell types with subclusterings. -->
<!-- cell_types_subclustering <- cell_types_subclustering[!cell_types_subclustering %in% "Coarse"] -->

<!-- for (cell_type in cell_types_subclustering){ -->
<!--   cat(paste0("\n\n## ", cell_type)) -->
<!--   plot_heatmap(cell_type) -->
<!-- } -->
...
plot_heatmap <- function(cell_type){
  ...
  # row_annotations <- heatmap_marker_genes_subclusters %>%
  #   filter(Cell.type==cell_type) %>%
  #   select("Gene", "Label") %>%
  #   rename("Subcluster"="Label") %>%
  ...
  # create the plot. We convert to ggplot to take advantage of some of the display functions
  #mat <- as.matrix(sce_plot)
  # hmap <- pheatmap(sce_plot, cluster_cols=FALSE, cluster_rows = FALSE, show_rownames=TRUE, show_colnames=FALSE, color=viridis(50),annotation_col=column_annotations, annotation_colors=annotation_colours,
...
```{r}
# pheatmap(mat, cluster_cols=FALSE, cluster_rows = FALSE, show_rownames=TRUE, show_colnames=FALSE, color=viridis(50), #,annotation_col=column_annotations, annotation_colors=annotation_colours,
#          heatmap_legend_param = list(title = "", direction = "vertical"),
#          top_annotation = HeatmapAnnotation(Subcluster=anno_block(gp = column_annotations$Subcluster)
#                                               ))
#            
#            
#            "Subcluster" = anno_block(gp = gpar(fill = 2:3), 
#                                                                       labels = c("group1", "group2"))))
# draw(hmap, annotation_legend_side="bottom", heatmap_legend_side="right")
# 
...
saveRDS(sce_fresh, file="../data/fresh_data/Protocol2_Fresh_all_sce_EOS3.rds")
