...
```{r}
sce <- readRDS(file = "../data/fresh_data/protocol_integration_analysis_sce_EOS1.rds")
# some row symbols are duplicated 
...
Many cell have to be reformatted with very large or small factors. We would normally expect the values to be normally distributed.

If we divide the expression values of each cell by its respective librarySizeFactor we get the same total expression count for all cells. The new total expression count will be what was formerly the average of the total expression values.

```{r Normalisation by library size}
lib_sce <- librarySizeFactors(sce)
hist(log10(lib_sce), xlab="Log10 of re-sizing factor", col="grey80", main="Distribution of factors to resize cells")
```

## Normalisation via deconvolution {.tabset}

```{r normalisation via deconvolution, fig.width=12, fig.height=10, resuts="asis", warning=FALSE}
#bpstart(bpparam)
clust.sce <- sce %>%  
  quickCluster(block=sce$Sampe) #BPPARAM=bpparam)
# bpstop(bpparam)

#bpstart(bpparam)
sce_deconv <- sce %>%  
  computeSumFactors(cluster=clust.sce) #, BPPARAM=bpparam)
#bpstop(bpparam)

# we overwrite the sce variable here as it can cause issues with parallel processing.
sce <- sce_deconv
...
plot(librarySizeFactors(sce), sizeFactors(sce), 
     ...
     ylab="Deconvolution size factor", 
     log='xy', pch=20,
     col=as.integer(factor(sce$Sample)),
     main="Deconvolution factors for individual samples")
abline(a=0, b=1, col="red")


```

## Apply normalisation to SCE

First we just apply the scaling factors.

```{r Apply normalisation, class.source="fold-show"}
sce <- logNormCounts(sce)
```

## Check for downsampling

We now check whether there are any issues due to the downsampling. Specifically, we are checking for a systematic trend of the log-counts with respect to the count size. We can observe this if there is a correlation between the major PC dimensions and the library size.

```{r downsampling, fig.width=12, fig.height=8}
# runPCA stores the first 50 dimension per default
set.seed(100)
sce <- runPCA(sce)
...
plotReducedDim(sce, "PCA", colour_by=I(sce$log_total), point_size=0.1) +
  ...
  scale_colour_gradientn(limits = c(0,5),  colours=c("navyblue", "darkmagenta", "darkorange1"),) +
  theme_ipsum_rc()

cor.test(reducedDim(sce, "PCA")[,"PC1"], sce$log_total)
```

*--> There is evidence for correlation between the PC 1 and the library size factors* 

We fix this by instead using scTransform
...
```{r}
saveRDS(sce, file = "../data/fresh_data/protocol_integration_analysis_sce_EOS2.rds")
```
