...
table(sce$Protocol, sce$manual_labels_coarse)
```
...
sce_reduced <- sce[,sce$manuals_labels_coarse]


# Cross-tabulate the label and the samples
abundances <- table(colData(sce)[[label]], colData(sce_reduced)[["Sample"]])
# returns a copy of the table with its table class attribute removed
abundances <- unclass(abundances) 
