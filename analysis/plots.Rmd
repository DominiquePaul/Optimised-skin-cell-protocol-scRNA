...
Fig2a <- plotReducedDim(sce_fresh, "UMAP", colour_by="manual_labels_coarse", text_by="manual_labels_numeric", point_size=0.25, text_size=7) +
  theme_ipsum_updated +
...
get_annotated_subcluster <- function(cell_type){
  ...
  # sce_subset$labels_numeric <- sce_subset$subcluster_names
  
  plt <- plotReducedDim(sce_subset, paste0("UMAP_", cell_type),
                 colour_by = "subcluster_names", text_by="subcluster_id", text_size=7) +
    theme_ipsum_updated +
...
# A
Fig4a <- plotReducedDim(sce_fvc, "UMAP", colour_by="manual_labels_coarse", text_by="manual_labels_numeric", point_size=0.25,
                        text_size=7) +
...
# A
Fig6a <- plotReducedDim(sce_integrated, "UMAP", colour_by="manual_labels_coarse", text_by="manual_labels_numeric", point_size=0.25, text_size=8) +
  # labs(title="UMAP coloured by cell type",
...
# the cells which we want to display, ordered by cell type
column_annotations <- data.frame(cell_name=rownames(colData(sce_fresh)), 
                                 Sample=colData(sce_fresh)[,"Sample"],
                                Cell_type=factor(colData(sce_fresh)[,"manual_labels_coarse"], levels=cell_type_order)) %>%
    filter(Cell_type!="NA") %>%
    arrange(Cell_type) %>%
    tibble::column_to_rownames(var="cell_name")
